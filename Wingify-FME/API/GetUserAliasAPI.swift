 ...
 * from the Wingify backend. It's used to resolve temporary user IDs to their permanent
...
class GetUserAliasAPI {
     ...
     *
     * This method makes an HTTP GET request to the Wingify backend to fetch alias
     ...
     * - `accountId`: Wingify account identifier for authentication
     ...
     * @param accountId Wingify account identifier
     ...
     */
    func getUserAlias(userIds: [String], accountId: Int, sdkKey: String, serviceContainer: ServiceContainer? = nil, loggerService: LoggerService? = nil, completion: @escaping (Result<GetUserAliasResponse, Error>) -> Void) {
        ...
        let requestModel = RequestModel(
            url:   UrlService.getBaseUrl(for: .get, serviceContainer: serviceContainer),
            method: HTTPMethod.get.rawValue,
            path: UrlEnum.getUserAlias.rawValue,
            query: queryParams,
            body: nil,
            headers: nil,
            scheme: serviceContainer?.getSettingsManager()?.protocolType ?? SettingsManager.instance?.protocolType ?? Constants.HTTPS_PROTOCOL,
            port: serviceContainer?.getSettingsManager()?.port ?? SettingsManager.instance?.port ?? 0,
