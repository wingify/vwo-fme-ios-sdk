...
class NetworkClient: NetworkClientInterface {
    ...
    private func logResolvedUrl(_ url: URL, method: HTTPMethod, request: RequestModel) {
        let message = "Resolved \(method.rawValue) URL: \(url.absoluteString)"

        if let accountId = request.accountId,
           let sdkKey = request.sdkKey,
           let logger = LoggerService.getInstance(accountId: accountId, sdkKey: sdkKey) {
            logger.log(level: .debug, message: message)
        } else {
            LoggerService.log(level: .debug, message: message)
        }
    }
    ...
    func POST(request: RequestModel, completion: @escaping (ResponseModel) -> Void) {
        ...
        logResolvedUrl(url, method: .post, request: newRequest)
    ...
    func GET(request: RequestModel, completion: @escaping (ResponseModel) -> Void) {
        ...
        logResolvedUrl(url, method: .get, request: newRequest)
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            if retryCount > 0 {
                ...
                if attempt > 1 && shouldSendDebugEvent(for: updatedRequestModel) {
                    ...
                    DebuggerServiceUtil.sendDebugEventToWingify(eventProps: removeNullValues(debugEventProps))
                }
