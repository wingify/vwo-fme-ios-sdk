...
class DecisionUtil {
    ...
    static func makeDecisionCustomVariables(context: WingifyUserContext, userId: String) -> [String: Any] {
        var decisionCustomVariables = [String: Any]()
        for (key, value) in context.customVariables {
            decisionCustomVariables[key] = value
        }
        decisionCustomVariables["_vwoUserId"] = userId
        return decisionCustomVariables
    }
     ...
     *   - context: The context of the current Wingify session.
    ...
    static func checkWhitelistingAndPreSeg(
        ...
        let decisionCustomVariables = makeDecisionCustomVariables(context: context, userId: userId ?? "")
        
        decision["customVariables"] = decisionCustomVariables
        ...
        let isPreSegmentationPassed = CampaignDecisionService().getPreSegmentationDecision(
            campaign: campaign,
            context: context,
            customVariables: decisionCustomVariables,
            serviceContainer: serviceContainer
        )
     ...
     */
    private static func checkCampaignWhitelisting(campaign: Campaign, context: WingifyUserContext, serviceContainer: ServiceContainer) -> [String: Any?]? {
     ...
     * - Returns: A dictionary containing the whitelisted variation, if any.
     */
    private static func evaluateWhitelisting(campaign: Campaign, context: WingifyUserContext, serviceContainer: ServiceContainer) -> [String: Any?]? {
