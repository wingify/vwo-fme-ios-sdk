...
struct UsageStatsValues {
    ...
    enum HybridSdk {
        ...
        var sdkName: String {
            switch self{
            case .reactNative:
                return ProductConfig.current.hybridReactNativeSdkName
            case .flutter:
                return ProductConfig.current.hybridFlutterSdkName
            }
        }
...
class UsageStatsUtil {
    ...
    private let queue = DispatchQueue(label: "com.wingify.usagestats.queue") // serial queue
    
    private init(storageService: StorageService) {
            self.localStorageService = storageService
        }
    
     func setUsageStats(options: WingifyInitOptions?) {
        queue.sync {
