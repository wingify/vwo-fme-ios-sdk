...
class WingifyClient {
    ...
    var options: WingifyInitOptions?
    var wingifyBuilder: WingifyBuilder?
    
    var isSettingsValid = false
    var settingsFetchTime: Int64 = 0
    
    // Initialize the WingifyClient with settings and options
    
    init(options: WingifyInitOptions?, settingObj: Settings?) {
    ...
    init(options: WingifyInitOptions?, settingObj: Settings?, wingifyBuilder: WingifyBuilder?) {
        self.options = options
        self.wingifyBuilder = wingifyBuilder
    ...
    func createServiceContainer() -> ServiceContainer? {
        guard let options = self.options,
              let wingifyBuilder = self.wingifyBuilder else {
            return nil
        }
        
        return wingifyBuilder.createServiceContainer(processedSettings: self.processedSettings, options: options)
    }
    ...
    func getFlag(featureKey: String?, context: WingifyUserContext, completion: @escaping (GetFlag) -> Void) {
        let apiName = "getFlag"
        let getFlag = GetFlag()
    ...
    private func track(eventName: String, context: WingifyUserContext?, eventProperties: [String: Any]) {
        ...
        do {
            ...
            guard DataTypeUtil.isString(eventName) else {
                ...
                throw NSError(domain: Constants.CLIENT_ERROR_DOMAIN, code: 400, userInfo: [NSLocalizedDescriptionKey: "TypeError: Event-name should be a string"])
            }
            ...
            let userId = UserIdUtil().getUserId(context: context, serviceContainer: serviceContainer)
            guard let userIdValue = userId.id, !userIdValue.isEmpty else {
                throw NSError(domain: Constants.CLIENT_ERROR_DOMAIN, code: 400, userInfo: [NSLocalizedDescriptionKey: Constants.userIdErrorMessage])
            }
    ...
    func trackEvent(eventName: String, context: WingifyUserContext?, eventProperties: [String: Any]) {
        track(eventName: eventName, context: context, eventProperties: eventProperties)
    }
    
    func trackEvent(eventName: String, context: WingifyUserContext?) {
        track(eventName: eventName, context: context, eventProperties: [:])
    }
    
    // Set attributes for a user in the context provided
    func setAttribute(attributes: [String: Any], context: WingifyUserContext?) {
