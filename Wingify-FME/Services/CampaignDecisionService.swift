...
class CampaignDecisionService {
    ...
    func getPreSegmentationDecision(
        ...
        if segments.isEmpty {
            ...
            let segmentationProperties = customVariables ?? context.customVariables
            let preSegmentationResult = serviceContainer.getSegmentationManager().validateSegmentation(
                dsl: segments,
