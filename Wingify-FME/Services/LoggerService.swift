...
class LoggerService {
    ...
    private static let instanceQueue = DispatchQueue(label: "com.wingify.fme.loggerservice.instances", attributes: .concurrent)
    private static var _instances: [String: LoggerService] = [:]
    
    // Thread-safe static message dictionaries
    private static let messageQueue = DispatchQueue(label: "com.wingify.fme.loggerservice.messages", attributes: .concurrent)
    ...
    private static func getLogTemplate(level: LogLevelEnum, key: String) -> String? {
        let template = getLogFile(level: level)[key]
        return ProductConfig.localizeLogMessage(template)
    }
    ...
    static func log(level: LogLevelEnum, key: String, details: [String: String]?) {
        let template = self.getLogTemplate(level: level, key: key)
        let messageBuilder = LogMessageUtil.buildMessage(template: template, data: details)
     ...
     func errorLog(key: String, data: [String: Any]? = nil, debugData: [String: Any]? = nil, shouldSendToVWO: Bool = true) {
        ...
        let template = LoggerService.getLogTemplate(level: .error, key: key)
        let message = LogMessageUtil.buildMessage(template: template, data: data) ?? "Unknown error"
    ...
    static func errorLog(key: String, data: [String: Any]? = nil, debugData: [String: Any]? = nil, shouldSendToVWO: Bool = true) {
        ...
        if let instance = getInstanceForStaticCall() {
            ...
            if shouldSendToVWO {
                ...
                DebuggerServiceUtil.sendDebugEventToWingify(eventProps: debugEventProps, serviceContainer: nil)
            }
        }
    }
    ...
    func log(level: LogLevelEnum, key: String, details: [String: String]?) {
        ...
        let template = LoggerService.getLogTemplate(level: level, key: key)
        let messageBuilder = LogMessageUtil.buildMessage(template: template, data: details)
