...
class AliasIdentifierManager {
    ...
    private static let instanceQueue = DispatchQueue(label: "com.wingify.fme.aliasmanager.instances", attributes: .concurrent)
     ...
     */
    func setServiceContainer(_ container: ServiceContainer, options: WingifyInitOptions?) {
     ...
     * - Parameter options: The WingifyInitOptions containing alias configuration
     */
    func setIsEnabled(options : WingifyInitOptions?) {
     ...
     * @param WingifyUserContext WingifyUserContext containing the user information and ID.
     ...
     */
    func setAlias(from wingifyUserContext: WingifyUserContext, to alias: String, serviceContainer: ServiceContainer? = nil) {
        ...
        guard let tempId = wingifyUserContext.id,!tempId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            if let logger = loggerService {
