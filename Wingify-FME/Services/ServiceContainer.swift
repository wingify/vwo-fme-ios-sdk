...
/**
 * ServiceContainer manages all services required for Wingify SDK operations.
 * This ensures each Wingify instance has its own isolated set of services,
 * preventing conflicts when multiple accounts are used.
 */
class ServiceContainer {
    private var settingsManager: SettingsManager?
    private let options: WingifyInitOptions
    ...
    init(settingsManager: SettingsManager?, options: WingifyInitOptions, settings: Settings?, loggerService: LoggerService?) {
    ...
    // MARK: - Options
    func getWingifyInitOptions() -> WingifyInitOptions {
        return options
    }
    ...
    // MARK: - Base URL
    func getBaseUrl(for method: HTTPMethod, ignoreGateway: Bool = false) -> String {
        let gatewayBaseUrl = self.settingsManager?.hostname ?? ""

        // If gateway service is provided, return base URL as-is.
        if !ignoreGateway && !self.options.gatewayService.isEmpty {
            return gatewayBaseUrl
        }

        let baseUrl = method == .get ? Constants.SERVING_URL : Constants.COLLECTION_URL

        // Collection prefixes apply only to collection/post requests.
        if method == .post,
           let collectionPrefix = self.settings?.collectionPrefix,
