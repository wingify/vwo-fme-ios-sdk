...
final class GetFlagTests: XCTestCase {
    ...
    var mockContext: WingifyUserContext!
    var mockHookManager: MockHooksManager!
    var mockBuilder: WingifyBuilder!
    var mockOptions: WingifyInitOptions!
    ...
    override func setUp() {
        ...
        mockContext = WingifyUserContext(id: "\(Date().timeIntervalSince1970)_user_id", customVariables: [:])
    ...
    func testGetFlagWithoutStorage() {
        ...
        if let myData = data, let cases = myData.withoutStorage {
            ...
            for item in cases {
                ...
                let mockClient = WingifyClient(options: mockOptions, settingObj: mockSettings)
                let processedMockSetting = mockClient.processedSettings!
                
                let contextData = WingifyUserContext(id: item.context.id!, customVariables: item.context.customVariables!)
                // Create ServiceContainer for this call
                let builder = WingifyBuilder(options: mockOptions)
    ...
    func testGetFlagWithSameSalt() {
        ...
        for item in userIds {
            ...
            let userContext = WingifyUserContext(id: item, customVariables: [:])
    ...
    func testGetFlagWithPresegmentLocationDeviceTypeOSPassWithTestingRule() {
        ...
        let testContext = WingifyUserContext(id: "pass_variation_user_id", customVariables: [:])
    ...
    func testGetFlagWithPresegmentLocationDeviceTypeOSPassWithRollout() {
        ...
        let testContext = WingifyUserContext(id: "\(Date().timeIntervalSince1970)_user_id", customVariables: [:])
        let mockSettings = FlagTestDataLoader.loadTestData(jsonFileName: SettingsTestJson.RolloutAndTestingSettingsWithPreSegmentMobile.jsonFileName)
        let mockOptions: WingifyInitOptions = WingifyInitOptions(sdkKey: "sdk-key", accountId: 123456)
        let mockClient = WingifyClient(options: mockOptions, settingObj: mockSettings)
        let processedMockSetting = mockClient.processedSettings!
    ...
    func testGetFlagWithPresegmentFeatureFlagStatus() {
        ...
        let mockClient = WingifyClient(options: mockOptions, settingObj: mockSettings)
        let processedMockSetting = mockClient.processedSettings!
        let userContext = WingifyUserContext(id: "user-id-feature-flag-status", customVariables: [:])
        let builder = WingifyBuilder(options: mockOptions)
        _ = builder.setLogger().setSettingsManager()
        let container = builder.createServiceContainer(processedSettings: processedMockSetting, options: mockOptions)
        GetFlagAPI.getFlag(featureKey: featureKey1, settings: processedMockSetting, context: userContext, hookManager: mockHookManager, serviceContainer: container) { getFlag1 in
    ...
    func testGetFlagWithCustomVariableInlist() {
        ...
        let mockClient = WingifyClient(options: mockOptions, settingObj: mockSettings)
        let processedMockSetting = mockClient.processedSettings!
        let userContext = WingifyUserContext(id: "user-id-inlist", customVariables: ["inlistvar":"user1"])
    ...
    func testGetFlagWithUserIdInlist() {
        ...
        let mockClient = WingifyClient(options: mockOptions, settingObj: mockSettings)
        let processedMockSetting = mockClient.processedSettings!
        let userContext = WingifyUserContext(id: userIdForContext, customVariables: [:])
