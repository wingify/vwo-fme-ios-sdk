...
- **User tracking for `getFlag`** — the SDK now tracks users when a feature flag is evaluated, even if the user is not added to a campaign.
- Works for both fresh evaluations and cached (stored) flag decisions.
